      ...
      <el-container>
        ...
        <el-aside :width="isCollapse ? '64px' : '200px'">
          <el-menu background-color="#545c64" text-color="#fff" active-text-color="#409EFF" unique-opened :collapse="isCollapse" :collapse-transition="false" router :default-active="nowActive">
            <!-- 一级菜单 -->
            <el-submenu :index="items.auth_id + ''" :key="items.id" v-for="items in menus">
              <!-- 一级菜单模板区 -->
              <template slot="title" >
                <!-- 图标 -->
                <i :class="icon[items.auth_id]"></i>
                <!-- 名称 -->
                <span>{{items.auth_name}}</span>
              </template>

              <!-- 二级菜单 -->
              <el-menu-item :index="'/'+childItem.path" :key="childItem.auth_id" v-for="childItem in items.children" @click="saveActive('/'+childItem.path)">
                <!-- 二级菜单模板区 -->
                <template slot="title" >
                  <!-- 图标 -->
                  <i class="el-icon-menu"></i>
                  <!-- 名称 -->
                  <span>{{childItem.auth_name}}</span>
                </template>
              </el-menu-item>
            </el-submenu>
          </el-menu>
...
export default {
  ...
  methods: {
    ...
    showMenu: async function () {
      // 查询菜单栏
      const result = await this.axios.get('auth/menu')
